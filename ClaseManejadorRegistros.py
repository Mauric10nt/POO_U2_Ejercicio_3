...
class ManejadorRegistros:
    ...
    def agregarRegistro(self, unRegistro, dia, hora):
        ...
        if isinstance(unRegistro, Registro):
            ...
            band = True
        return band
    # cargo paso las comas como punto...
    ...
    def getDiaHoraMenorTemperatura(self):
        ...
        for i in range(30):# 30 dias 
            for j in range(24):# 24 horas
                if self.__lista[dia-1][hora].gettemp() > self.__lista[i][j].gettemp():
                    dia = i + 1
                    hora = j
        return (dia, hora)
    ...
    def getDiaHoraMenorHumedad(self):
        ...
        for i in range(30):
            for j in range(24):
                if self.__lista[dia-1][hora].gethume() > self.__lista[i][j].gethume():
                    dia = i + 1
                    hora = j
        return (dia, hora)
    ...
    def getDiaHoraMenorPresion(self):
        ...
        for i in range(30):
            for j in range(24):
                if self.__lista[dia-1][hora].getpres() > self.__lista[i][j].getpres():
                    dia = i + 1
                    hora = j
        return (dia, hora)
    ...
    def mostrarTodosMayorMenor(self):
        ...
        print("Dia y hora con mayor temperatura: {0} a las {1}hs con {2}°C".format(diaMayorTemperatura, horaMayorTemperatura, self.__lista[diaMayorTemperatura-1][horaMayorTemperatura].gettemp()))
        print("Dia y hora con menor temperatura: {0} a las {1}hs con {2}°C".format(diaMenorTemperatura, horaMenorTemperatura, self.__lista[diaMenorTemperatura-1][horaMenorTemperatura].gettemp()))
        print("Dia y hora con mayor humedad: {0} a las {1}hs con {2}%".format(diaMayorHumedad, horaMayorHumedad, self.__lista[diaMayorHumedad-1][horaMayorHumedad].gethume()))
        print("Dia y hora con menor humedad: {0} a las {1}hs con {2}%".format(diaMenorHumedad, horaMenorHumedad, self.__lista[diaMenorHumedad-1][horaMenorHumedad].gethume()))
        print("Dia y hora con mayor presion: {0} a las {1}hs con {2}mmHg".format(diaMayorPresion, horaMayorPresion, self.__lista[diaMayorPresion-1][horaMayorPresion].getpres()))
        print("Dia y hora con menor presion: {0} a las {1}hs con {2}mmHg".format(diaMenorPresion, horaMenorPresion, self.__lista[diaMenorPresion-1][horaMenorPresion].getpres()))
    ...
    def calcularTemperaturaPromedioHora(self, hora:int):
        ...
        if isinstance(hora, int) and 0<=hora<=23:
            total = 0
            for i in range(30):
                total += self.__lista[i][hora].gettemp()
            promedio = total / 30
        
        return promedio
    
    def mostrarTemperaturaPromedioTodasHoras(self):
        print("Temperaturas promedio mensual".center(80, "*"))
    ...
    # valores de un dia..formato
    def listarValoresUnDia(self):
        dia = int(input("Ingrese el dia: "))
        if 1<=dia<=30:
            print(("Valores del dia {0}".format(dia)).center(80, "*"))
            print("{0:^15}{1:^15}{2:^15}{3:^15}".format("Hora", "Temperatura", "Humedad", "Presion"))
            for hora in range(24):
                print("{0:^15}{1:^15.2f}{2:^15.2f}{3:^15.2f}".format(hora, self.__lista[dia-1][hora].gettemp(), self.__lista[dia-1][hora].gethume(), self.__lista[dia-1][hora].getpres()))
        else:
